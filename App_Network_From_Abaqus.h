...
public:
    ...
    int Apply_displacements_to_gnps(const vector<int>& gnps_outside, const vector<vector<int> >& vertices_gnps_out, const vector<vector<bool> >& vertex_flags, const vector<GNP>& gnps0, odb_Assembly& root_assy, odb_FieldOutput& current_fieldU, vector<GNP>& gnps)const;
    vector<int> All_gnp_vertices()const;
    int Apply_displacements_to_gnp_vertices(const vector<int>& vertices, odb_Assembly& root_assy, odb_FieldOutput& current_fieldU, GNP& gnp_i)const;
