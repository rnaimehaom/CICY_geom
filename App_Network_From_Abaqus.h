...
public:
    ...
    int Apply_displacements_to_sample(odb_Assembly& root_assy, odb_FieldOutput& fieldU, Geom_sample& geom_sample)const;
    int Apply_displacements_to_cnts(const vector<vector<long int> >& structure, odb_Assembly& root_assy, odb_FieldOutput& fieldU, const int& n_cnts, vector<Point_3D>& points_cnt)const;
