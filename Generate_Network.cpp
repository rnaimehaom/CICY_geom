...
int Generate_Network::Generate_nanoparticle_network(const Simu_para &simu_para, const Geom_sample &geom_sample, const Nanotube_Geo &nanotube_geo, const GNP_Geo &gnp_geo, const Cutoff_dist &cutoffs, const Visualization_flags &vis_flags, const Output_data_flags &out_flags, vector<Point_3D> &points_cnt, vector<double> &radii_out, vector<vector<long int> > &structure, vector<GNP> &gnps)const
{
    ...
    if (simu_para.particle_type == "CNT_wires") {
        ...
        if (!Generate_gnp_network_mt(simu_para, gnp_geo, geom_sample, cutoffs, gnps, sectioned_domain_gnp, n_subregion, gnp_vol_tot, gnp_wt_tot)) {
        ...
        //Variables for the sectioned domain, which needs to have the same size for 
        //CNTs and GNPs
        vector<vector<int> > sectioned_domain_gnp;
        int n_subregion[] = { 0,0,0 };
        ...
        if (!Generate_cnt_network_threads_among_gnps_mt(simu_para, gnp_geo, nanotube_geo, geom_sample, cutoffs, gnps, sectioned_domain_gnp, n_subregion, gnp_vol_tot, gnp_wt_tot, cnts_points, radii_in)) {
    ...
    if (vis_flags.generated_nanoparticles) {
        ...
        if (gnps.size()) {
            ...
            if (vis_flags.generated_nanoparticles == 1 || vis_flags.generated_nanoparticles == 3)
            {
...
int Generate_Network::Generate_cnt_network_threads_mt(const Simu_para &simu_para, const Geom_sample &geom_sample, const Nanotube_Geo &nanotube_geo, const Cutoff_dist &cutoffs, vector<vector<Point_3D> > &cnts_points, vector<double> &cnts_radius)const
{
    ...
    int tot_regions = n_subregions[0] * n_subregions[1] * n_subregions[2];
    ...
    while(!terminate)
    {
        ...
        //Calculate the number of points to be ignored for self-penetration
        //Make sure it is at least 1
        int ignr = max(1, (int)(cnt_cutoff / nanotube_geo.step_length));
        ...
        //First 0 is the local number of the seed point on the CNT
        //Second 0 is to indicate no layer is considered when calculating subregion numbers
        if (!Add_cnt_point_to_overlapping_regions_map(geom_sample, new_cnt[0], 0, is_prev_in_sample, 0, tot_regions, n_subregions, subr_point_map)) {
        ...
        for(int i = 0; i < step_num; i++)
        {
            ...
            if(Is_point_inside_cuboid(geom_sample.ex_dom_cnt, new_point))
            {
                ...
                if (!simu_para.penetration_model_flag || penetration_check == 1) {
                    ...
                    //0 is to indicate no layer is considered when calculating subregion numbers
                    if (!Add_cnt_point_to_overlapping_regions_map(geom_sample, new_point, (int)new_cnt.size(), is_new_inside_sample, 0, tot_regions, n_subregions, subr_point_map)) {
...
int Generate_Network::Check_penetration(
    const int &MAX_ATTEMPTS_CNT, 
    const Geom_sample &geom_sample, 
    const Nanotube_Geo &nanotube_geo, 
    const vector<vector<Point_3D> > &cnts, 
    const vector<vector<int> > &global_coordinates, 
    const vector<vector<long int> > &sectioned_domain, 
    const vector<double> &radii, 
    const vector<Point_3D> &new_cnt, 
    const int n_subregions[], 
    const double &rad_p_dvdw, 
...
int Generate_Network::Get_cnt_point_subregion_with_layer(const Geom_sample& geom_sample, const int n_subregions[], const Point_3D& point)const
{
    //Check point is in non-penetration domain
    if (point.is_outside_cuboid(geom_sample.np_domain))
    {
        //point is in the boudary layer but not in the non-penetration layer, so interpenetrations
        //are ignored. thus return -1
        return -1;
    }
    //Calcualte the subregion with layer
    else 
    {
        //These variables will give me the region cordinates of the region that a point belongs to
        int a, b, c;
        ...
        //Calculate the subregion-coordinates
        Get_subregion_coordinates_with_layer(geom_sample, point, n_subregions, a, b, c);
...
{   
    ...
    for (long int i = 0; i < (long int)subregion_vec.size(); i++) {
        ...
        long int coord2 = subregion_vec[i];
        //hout << "coord2="<<coord2<<" global_coordinates.size()="<<global_coordinates.size()<<endl;
        
        int CNT2 = global_coordinates[coord2][0];
        int P2 = global_coordinates[coord2][1];

        //hout << "Check4 ";
        //Cutoff distance between two points (of different CNTs)
        double cutoff_p = rad_plus_dvdw + radii[CNT2];
        ...
        if (distance + Zero - cutoff_p < Zero)
        {
            ...
            //hout << "Penetrating point"<< affected_points.size()<< " CNT2=" << CNT2 << "("<<cnts[CNT2].size()<<" points) P2=" << P2 << " r2=" << radii[CNT2] << " P2" << cnts[CNT2][P2].str() << endl;
        }
        //hout << "Check5 ";
    }
    //hout << "Check6 " << endl;
...
void Generate_Network::Get_penetrating_points_within_cnt(const int &subregion, const int& ignr, const double &cnt_cutoff, const double &cnt_cutoff2, const Point_3D &point, const vector<Point_3D> &cnt_new, const map<int, vector<int> > &subr_point_map, vector<Point_3D> &affected_points, vector<double> &cutoffs_p, vector<double> &distances)const
{
    ...
    if (it != subr_point_map.end() && subr_point_map.at(subregion).size()) {
        ...
        int prev_last_idx = (int)cnt_new.size() - ignr;
        //hout << "prev_last_idx=" << prev_last_idx <<" last_idx="<< new_cnt.size()-1 << endl;
        ...
        for (size_t i = 0; i < subr_point_map.at(subregion).size(); i++) {
            ...
            //Get the point number within new_cnt
            ...
            if (Pi < prev_last_idx) {
                ...
                if (dist + Zero - cnt_cutoff2< Zero) {
                    ...
                    //hout<<"aff_pt"<<new_cnt[Pi].str()<<" dist="<<sqrt(dist)<<" cnt_cutoff="<<cnt_cutoff<<endl;
...
int Generate_Network::Move_point_by_rotating_cnt_segment(const double &step, const vector<Point_3D> &cnt_new, const vector<double> &cutoffs, const vector<double> &distances, const vector<Point_3D> &affected_points, Point_3D &new_point)const
{
    ...
    if (affected_points.size() == 1){
        ...
        if (!One_penetrating_point(step, cutoffs[0], affected_points[0], cnt_new.back(), new_point)) {
            hout<<"Error in Move_point_by_rotating_cnt_segment when calling One_penetrating_point"<<endl;
            return 0;
        }
        ...
        if (!Two_penetrating_points(step, cutoffs, affected_points, new_cnt.back(), new_point)) {
            hout<<"Error in Move_point_by_rotating_cnt_segment when calling Two_penetrating_points"<<endl;
            return 0;
        }
    } */
    else if (affected_points.size() >= 2) {
        ...
        if (!Three_or_more_penetrating_points(step, cutoffs, distances, affected_points, new_cnt.back(), new_point)) {
            hout<<"Error in Move_point_by_rotating_cnt_segment when calling Three_or_more_penetrating_points"<<endl;
            return 0;
        }*/
        ...
        if (!Two_or_more_penetrating_points(step, cutoffs, distances, affected_points, cnt_new.back(), new_point)) {
            hout<<"Error in Move_point_by_rotating_cnt_segment when calling Two_or_more_penetrating_points"<<endl;
            return 0;
        }
    }
...
int Generate_Network::One_penetrating_point(const double &d_s, const double &d_c, const Point_3D &S, const Point_3D &P0, Point_3D &N)const
{
    ...
    {
        hout << "Error in One_penetrating_point. Attempting to take the square root of a negative number: sqrt("<<tmp<<")" << endl;
        hout << "S=" << S.str() << endl;
        hout << "P0=" << P0.str() << endl;
        hout << "N=" << N.str() << endl;
        hout << "dp=" << N.distance_to(S) << endl;
        hout << "d_s=" << d_s << " d1=" << d1 << " d_c=" << d_c << endl;
        hout << "d_s*d_s=" << d_s2 << " d1*d1=" << d1 * d1 << " d_c*d_c=" << d_c * d_c << endl;
        return 0;
    }
...
int Generate_Network::Add_cnt_point_to_overlapping_regions_map(const Geom_sample &geom_sample, const Point_3D &new_point, const int &local_num, const int &is_new_inside_sample, const int& layer_flag, const int& tot_regions, const int n_subregions[], map<int, vector<int> > &subr_point_map)const
{
    ...
    if (is_new_inside_sample) {
        ...
        //Variables to store the subregion coordinates
        int a = 0, b = 0, c = 0;
        
        //Initialize flags for overlaping regions
        int f_regions[] = { 0,0,0 };

        //Check if layer needs to be considered when calculating the subregion number
        if (layer_flag)
        {
            //Layer is present
            //Calculate the subregion-coordinates a, b, c
            if (!Get_subregion_coordinates_with_layer(geom_sample, new_point, n_subregions, a, b, c))
            {
                hout << "Error in Add_cnt_point_to_overlapping_regions_map when calling Get_subregion_coordinates_with_layer." << endl;
                return 0;
            }
        ...
        else
        {
            ...
            //Calculate the subregion-coordinates
            Get_subregion_coordinates(geom_sample, n_subregions, new_point, a, b, c);

            //Get the overlapping flags
            if (!Get_overlapping_flags(geom_sample, new_point, n_subregions, a, b, c, f_regions))
            {
                hout << "Error in Add_cnt_point_to_overlapping_regions_map when calling Get_overlapping_flags." << endl;
                return 0;
            }
        }
    ...
    if(new_cnt.size() >= 2 && points_in) {
        ...
        if (penetration_model_flag) {
            ...
            for(int ii = 0; ii < (int)new_cnt.size(); ii++) {
                ...
                if (!Add_cnt_point_to_overlapping_regions(geom_sample, new_cnt[ii], (long int)global_coordinates.size() - 1, tot_regions, n_subregions, sectioned_domain))
                {
                    hout << "Error in Store_or_ignore_new_cnt when calling Add_cnt_point_to_overlapping_regions" << endl;
                    return 0;
                }                
            }
        }
...
int Generate_Network::Add_cnt_point_to_overlapping_regions(const Geom_sample &geom_sample, const Point_3D &point, const long int &global_num, const int& tot_regions, const int n_subregions[], vector<vector<long int> > &sectioned_domain)const
{
    ...
    if (Is_point_inside_cuboid(geom_sample.sample, point)) {
        ...
        //Get the overlapping flags
        if (!Get_overlapping_flags(geom_sample, point, n_subregions, a, b, c, f_regions))
        {
            hout << "Error in Add_cnt_point_to_overlapping_regions when calling Get_overlapping_flags." << endl;
            return 0;
        }
        ...
        if (t >= tot_regions || t < 0)
        {
            hout << "Error in Add_cnt_point_to_overlapping_regions. Point belongs to a subregion outside the range of subregions." << endl;
...
int Generate_Network::Get_overlapping_flags(const Geom_sample& geom_sample, const Point_3D& new_point, const int n_subregions[], const int& a, const int& b, const int& c, int f_regions[])const
{
    ...
    if ((a > 0) && (new_point.x >= x1) && (new_point.x <= x1 + geom_sample.gs_overlap_cnt))
        f_regions[0] = -1;
    else if ((a < n_subregions[0] - 1) && (new_point.x >= x2 - geom_sample.gs_overlap_cnt) && (new_point.x <= x2))
        f_regions[0] = 1;
    if ((b > 0) && (new_point.y >= y1) && (new_point.y <= y1 + geom_sample.gs_overlap_cnt))
        f_regions[1] = -1;
    else if ((b < n_subregions[1] - 1) && (new_point.y >= y2 - geom_sample.gs_overlap_cnt) && (new_point.y <= y2))
        f_regions[1] = 1;
    if ((c > 0) && (new_point.z >= z1) && (new_point.z <= z1 + geom_sample.gs_overlap_cnt))
        f_regions[2] = -1;
    else if ((c < n_subregions[2] - 1) && (new_point.z >= z2 - geom_sample.gs_overlap_cnt) && (new_point.z <= z2))
        f_regions[2] = 1;
...
int Generate_Network::Generate_cnt_deposit_mt(const Simu_para &simu_para, const Geom_sample &geom_sample, const Geom_sample &geom_sample_deposit, const Nanotube_Geo &nanotube_geo, const Cutoff_dist &cutoffs, vector<vector<Point_3D> > &cnts_points, vector<double> &cnts_radii)const
{
    ...
    while(!terminate)
    {
        ...
        if (!Store_or_ignore_new_cnt(geom_sample_deposit, simu_para.penetration_model_flag, (int)new_cnt.size(), cnt_len, cnt_rad, cnt_cross_area, new_cnt, cnts_points, cnts_radii, tot_regions, n_subregions, sectioned_domain, global_coordinates, vol_sum, cnt_ignore_count)) {
            hout<<"Error when storing or ignoring a new CNT"<<endl;
            return 0;
        }
...
//---------------------------------------------------------------------------
//Generate a GNP network
int Generate_Network::Generate_gnp_network_mt(const Simu_para &simu_para, const GNP_Geo &gnp_geo, const Geom_sample &geom_sample, const Cutoff_dist &cutoffs, vector<GNP> &gnps, vector<vector<int> > &sectioned_domain, int n_subregion[], double &gnp_vol_tot, double &gnp_wt_tot)const
{
    ...
    //n_subregion[0] is the number of subregions along x
    //n_subregion[1] is the number of subregions along y
    //n_subregion[2] is the number of subregions along z
    ...
    int tot_regions = n_subregion[0] * n_subregion[1] * n_subregion[2];
...
{   
    //Calculate the number of variables along each direction
    //Make sure there is at least one subregion along each direction
    //The addition of 2 corresponds to the points that will lie outside the sample
    //One for points below the minimum coordinate and one for points beyond
...
int Generate_Network::Deal_with_gnp_interpenetrations(const int& MAX_ATTEMPTS_GNP, const int& tot_regions, const Geom_sample &geom_sample, const Cutoff_dist &cutoffs, const vector<GNP> &gnps, const int n_subregions[], GNP &gnp_new, set<int> &subregions_gnp, vector<vector<int> > &sectioned_domain, bool &rejected)const
{
...
//This function adds a GNP to the subregions it occupies
//Subregions outside the sample are also considered to avoid interpenetrations even outside
//the sample
int Generate_Network::Add_gnp_subregions_to_set_for_gnp_point(const int& tot_regions, const Geom_sample &geom_sample, const Point_3D &new_point, const int n_subregions[], set<int> &subregions)const
{
    //=================================================================================
    //Calculate the subregion-coordinates a, b, c
    int a = 0, b = 0, c = 0;
    if (!Get_subregion_coordinates_with_layer(geom_sample, new_point, n_subregions, a, b, c))
    {
        hout << "Error in Add_gnp_subregions_to_set_for_gnp_point when calling Get_subregion_coordinates_with_layer." << endl;
        return 0;
    }
    ...
    if (t >= tot_regions || t < 0)
    {
        hout << "Error in Add_gnp_subregions_to_set_for_gnp_point. Point belongs to a subregion outside the range of subregions." << endl;
        hout << "Subregion number (t) is " << t << ". Maximum number of subregions is " << tot_regions << endl;
    ...
    //Add the subregion number to the set of subregions
    subregions.insert(t);
...
//This function calculates the subregion coordinates when there is a boundary layer, i.e., there
//are subregions outside the sample
int Generate_Network::Get_subregion_coordinates_with_layer(const Geom_sample& geom_sample, const Point_3D& new_point, const int n_subregions[], int& a, int& b, int& c)const
{
    //Calculate the subregion coordinate along the x-direction
    a = (int)((new_point.x - geom_sample.sample.poi_min.x) / geom_sample.gs_minx);
    //Limit the value of a as it has to go from 0 to n_subregions[0]-1
    if (a < 0) a = 0;
    else if (a >= n_subregions[0]) a = n_subregions[0] - 1;

    //Calculate the subregion coordinate along the y-direction
    b = (int)((new_point.y - geom_sample.sample.poi_min.y) / geom_sample.gs_miny);
    //Limit the value of b as it has to go from 0 to n_subregions[1]-1
    if (b < 0) b = 0;
    else if (b >= n_subregions[1]) b = n_subregions[1] - 1;

    //Calculate the subregion coordinate along the z-direction
    c = (int)((new_point.z - geom_sample.sample.poi_min.z) / geom_sample.gs_minz);
    //Limit the value of c as it has to go from 0 to n_subregions[2]-1
    if (c < 0) c = 0;
    else if (c >= n_subregions[2]) c = n_subregions[2] - 1;
    //hout<<"a="<<a<<" b="<<b<<" c="<<c<<endl;
...
int Generate_Network::Generate_cnt_network_threads_among_gnps_mt(const Simu_para &simu_para, const GNP_Geo &gnp_geo, const Nanotube_Geo &nanotube_geo, const Geom_sample &geom_sample, const Cutoff_dist &cutoffs, const vector<GNP> &gnps, const vector<vector<int> > &sectioned_domain_gnp, const int n_subregions[], const double &gnp_vol_tot, const double &gnp_wt_tot, vector<vector<Point_3D> > &cnts_points, vector<double> &cnts_radius)const
{
    ...
    //sectioned_domain[i] contains all the points in sub-region i.
    //Sub-region i is an overlapping subregion to check for penetrations
    //Use the same size as for GNPs
    vector<vector<long int> > sectioned_domain(sectioned_domain_gnp.size());

    //For n_subregions use the same as with GNPs since the sectioned domain needs
    ...
    //Variable used to store the fraction of CNT volume indicated by vol_completed
    double vol_completed_acc = vol_completed*nanotube_geo.volume;
    //hout << "nanotube_geo.volume=" << nanotube_geo.volume<<" nanotube_geo.volume_fraction="<< nanotube_geo.volume_fraction << endl;
    ...
    while(!terminate)
    {
        //hout<<endl<<"CNTs generated:"<<cnts_points.size()<<endl;
        //---------------------------------------------------------------------------
        //Vector for a new nanotube
        vector<Point_3D> new_cnt;
        ...
        Point_3D new_point;
        if (Generate_initial_point_of_cnt(geom_sample, simu_para, cnts_points, cnts_radius, new_cnt, rad_p_dvdw, cnt_cutoff, cnt_cutoff2, nanotube_geo.step_length, ignr, subr_point_map, global_coordinates, sectioned_domain, gnps, sectioned_domain_gnp, n_subregions, new_point, engine_x, engine_y, engine_z, dist) != 1) {
        ...
        //Get the location of the initial point with respect to the sample
        bool is_prev_in_sample = Is_point_inside_cuboid(geom_sample.sample, new_cnt[0]);
        ...
        //Add the seed point to the overlapping subregions it belongs to using the map
        //0 is the local number of the seed point on the CNT
        //1 is to indicate no layer is considered when calculating subregion numbers
        //hout << "Add_cnt_point_to_overlapping_regions_map" << endl;
        if (!Add_cnt_point_to_overlapping_regions_map(geom_sample, new_cnt[0], 0, is_prev_in_sample, 1, tot_regions, n_subregions, subr_point_map)) {
            hout<<"Error when adding the seed point to the map of subregions"<<endl;
            return 0;
        }
        ...
        for(int i = 0; i < step_num; i++)
        {
            ...
            if(Is_point_inside_cuboid(geom_sample.ex_dom_cnt, new_point))
            {
                ...
                if (simu_para.penetration_model_flag) {
                    ...
                    mixed_interpenetration = Check_mixed_interpenetration(simu_para.MAX_ATTEMPTS_CNT, geom_sample, cnts_points, cnts_radius, new_cnt, rad_p_dvdw, cnt_cutoff, cnt_cutoff2, nanotube_geo.step_length, ignr, is_prev_in_np, subr_point_map, global_coordinates, sectioned_domain, gnps, sectioned_domain_gnp, n_subregions, new_point);
                ...
                //if (!penetration_model_flag || mixed_interpenetration) {
                    ...
                    cnt_len = cnt_len + Length_inside_sample(geom_sample.sample, new_cnt.back(), new_point, is_prev_in_sample, is_new_inside_sample);
                    //hout << "cnt_len=" << cnt_len << " prev="<< new_cnt.back().str()<<" in=" << is_prev_in_sample << " new="<<new_point.str()<<" in=" << is_new_inside_sample << endl;
                    ...
                    //For the next iteration of the for loop, new_point will become the previous point,
                    //so update the boolean is_prev_in_sample for the next iteration
                    is_prev_in_sample = is_new_inside_sample;
...
int Generate_Network::Generate_initial_point_of_cnt(
    ...
    mt19937 &engine_y, 
    mt19937 &engine_z, 
    uniform_real_distribution<double> &dist)const
{
    ...
    while (attempts <= simu_para.MAX_ATTEMPTS_CNT) {
        ...
        if (simu_para.penetration_model_flag) {
            ...
            //if there are interpenetrations
            //hout<<"Check_mixed_interpenetration"<<endl;
            int interpenetration = Check_mixed_interpenetration(simu_para.MAX_ATTEMPTS_CNT, geom_sample, cnts, radii, new_cnt, rad_plus_dvdw, cnt_cutoff, cnt_cutoff2, step_length, ignr, false, subr_point_map, global_coordinates, sectioned_domain_cnt, gnps, sectioned_domain_gnp, n_subregions, new_point);
...
int Generate_Network::Check_mixed_interpenetration(
    const int& MAX_ATTEMPTS_CNT,
    const Geom_sample& geom_sample,
    const vector<vector<Point_3D> >& cnts,
    const vector<double>& radii,
...
int Generate_Network::Move_point_or_rotate_segment(const Geom_sample& geom_sample, const bool& is_prev_in_np, const double& step_length, const vector<double>& cutoffs_p, const vector<double>& distances, const vector<Point_3D>& affected_points, vector<Point_3D>& new_cnt, Point_3D& new_point)const
{
    ...
    if (new_cnt.empty() || !is_prev_in_np)
    {
        ...
        //hout<<"Move_point_by_rotating_cnt_segment cnt.back="<<((new_cnt.empty())?" ":new_cnt.back().str())<<endl;
        //hout<<"new_point="<<new_point.str()<<endl;
        if (!Move_point_by_rotating_cnt_segment(step_length, new_cnt, cutoffs_p, distances, affected_points, new_point))
        {
            hout << "Error in Move_point_or_rotate_segment when calling Move_point_by_rotating_cnt_segment" << endl;
            return -1;
        }
        //hout<<"new_point="<<new_point.str()<<endl;
    }
