...
int App_Network_From_Abaqus::Apply_displacements_to_gnps(const vector<int>& gnps_outside, const vector<vector<int> >& vertices_gnps_out, const vector<vector<bool> >& vertex_flags, odb_Assembly& root_assy, odb_FieldOutput& previous_fieldU, odb_FieldOutput& current_fieldU, vector<GNP>& gnps)const
{
    ...
    for (int i = 0; i < n_gnps; i++)
    {
        ...
        else 
        {
            ...
            if (!Reconstruct_full_gnp(gnps[i]))
            {
                hout << "Error in Apply_displacements_to_gnps when calling Reconstruct_full_gnp" << endl;
                return 0;
            }
        }
    }
...
int App_Network_From_Abaqus::Reconstruct_full_gnp(GNP& gnp_i)const
{
...
int App_Network_From_Abaqus::Get_plane_from_top_squared_face(GNP& gnp_i, Plane_3D& Pl_top)const
{
...
int App_Network_From_Abaqus::Get_plane_from_bottom_squared_face(const Plane_3D& Pl_top, GNP& gnp_i, Plane_3D& Pl_bot, double& d_planes)const
{
...
int App_Network_From_Abaqus::Fit_squared_faces_on_parallel_planes(const Plane_3D& Pl_top, const Plane_3D& Pl_bot, double& d_planes, GNP& gnp_i)const
{
...
int App_Network_From_Abaqus::Adjust_reference_edge(const GNP& gnp_i, const Point_3D& N_top, const double& d_planes, Point_3D& R1, Point_3D& R2, Point_3D& R1R2_hat, Point_3D& N_r1r2)const
{
...
int App_Network_From_Abaqus::Get_gnp_side_length(const GNP& gnp_i, const Point_3D& M, const Point_3D& R1R2_hat, const Point_3D& N_r1r2, double& l_gnp)const
{
...
int App_Network_From_Abaqus::Recalculate_gnp_vertices(const Point_3D& M, const Point_3D& R1R2_hat, const Point_3D& N_r1r2, const Point_3D& N_bot, const double& l_gnp, const double& d_planes, GNP& gnp_i)const
{
...
int App_Network_From_Abaqus::Update_gnp_center(GNP& gnp_i)const
{
