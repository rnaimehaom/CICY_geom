...
public:
    ...
    //Member Functions
    int Generate_shells(const struct Geom_RVE &sample, const struct Nanotube_Geo &cnts, const vector<Point_3D> &points_in, const vector<GCH> &hybrid_particles, vector<vector<int> > &shells_cnt, vector<vector<int> > &shells_gnps);
    int Add_to_shell(const struct Geom_RVE &sample, const Point_3D &point, vector<vector<int> > &shells_cnt);
    int Find_minimum_shell(const struct Geom_RVE &sample, const Point_3D &point, const int &num_shells);
    ...
    int Add_to_shells(const struct Geom_RVE &sample, const GCH &hybrid, vector<vector<int> > &shells_gnp);
