...
public:
    ...
    //Member Functions
    int Determine_percolated_clusters(const int &window, const string &particle_type, const struct Geom_sample &sample, const struct Nanotube_Geo &cnts, const struct GNP_Geo &gnps, const vector<vector<int> > &boundary_cnt, const vector<int> &labels_cnt, const vector<vector<int> > &boundary_gnp, const vector<int> &labels_gnp, vector<vector<int> > &clusters_cnt, vector<vector<int> > &isolated_cnt, vector<vector<int> > &clusters_gnp, vector<vector<int> > &isolated_gnp);
    ...
    int Single_particle_percolation(const int &window, const string &particle_type, const struct Geom_sample &sample, const struct Nanotube_Geo &cnts, const struct GNP_Geo &gnps, const vector<vector<int> > &boundary, const vector<int> &labels, vector<vector<int> > &clusters, vector<vector<int> > &isolated);
    int Determine_direction_flags(const int &window, const struct Geom_sample &sample, const double &max_length, int &px, int &py, int &pz);
