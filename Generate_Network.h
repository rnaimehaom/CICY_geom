...
public:
    ...
    int Check_penetration(const int& MAX_ATTEMPTS_CNT, const Geom_sample &geom_sample, const Nanotube_Geo &nanotube_geo, const vector<vector<Point_3D> > &cnts, const vector<vector<int> > &global_coordinates, const vector<vector<long int> > &sectioned_domain, const vector<double> &radii, const vector<Point_3D> &new_cnt, const int n_subregions[], const double &rad_p_dvdw, const double &cnt_cutoff, const double &cnt_cutoff2, const int& ignr, const bool& is_prev_in_np, const map<int, vector<int> > &subr_point_map, int &point_overlap_count, int &point_overlap_count_unique, Point_3D &point)const;
    int Get_cnt_point_subregion(const Geom_sample &geom_sample, const int n_subregions[], const Point_3D &point)const;
    int Get_cnt_point_subregion_with_layer(const Geom_sample& geom_sample, const int n_subregions[], const Point_3D& point)const;
    ...
    void Get_penetrating_points_within_cnt(const int &subregion, const int& ignr, const double &cnt_cutoff, const double &cnt_cutoff2, const Point_3D &point, const vector<Point_3D> &cnt_new, const map<int, vector<int> > &subr_point_map, vector<Point_3D> &affected_points, vector<double> &cutoffs_p, vector<double> &distances)const;
    int Move_point_by_rotating_cnt_segment(const double &step, const vector<Point_3D> &cnt_new, const vector<double> &cutoffs, const vector<double> &distances, const vector<Point_3D> &affected_points, Point_3D &new_point)const;
    ...
    int Add_cnt_point_to_overlapping_regions_map(const Geom_sample &geom_sample, const Point_3D &new_point, const int &local_num, const int &is_new_inside_sample, const int &layer_flag, const int& tot_regions, const int n_subregions[], map<int, vector<int> > &subr_point_map)const;
    ...
    int Add_cnt_point_to_overlapping_regions(const Geom_sample &geom_sample, const Point_3D &point, const long int &global_num, const int& tot_regions, const int n_subregions[], vector<vector<long int> > &sectioned_domain)const;
    int Calculate_t(const int &a, const int &b, const int &c, const int &sx, const int &sy)const;
    int Get_overlapping_flags(const Geom_sample& geom_sample, const Point_3D& new_point, const int n_subregions[], const int& a, const int& b, const int& c, int f_regions[])const;
    ...
    int Rotation_matrix_from_direction(const double& a, const double& b, const double& c, MathMatrix& rotation)const;
    ...
    //---------------------------------------------------------------------------
    //GNPs
    int Generate_gnp_network_mt(const Simu_para &simu_para, const GNP_Geo &gnp_geo, const Geom_sample &geom_sample, const Cutoff_dist &cutoffs, vector<GNP> &gnps, vector<vector<int> > &sectioned_domain, int n_subregion[], double &gnp_vol_tot, double &gnp_wt_tot)const;
    ...
    int Get_gnp_penetrating_points(const double& step_lenght, const vector<GNP> &gnps, const vector<int> &subregion_gnp, const vector<Point_3D>& new_cnt, const double &cutoff, vector<Point_3D> &affected_points, vector<double> &cutoffs_p, vector<double> &distances, GNP &affected_gnp, Point_3D& new_point)const;
    int Move_point_or_rotate_segment(const Geom_sample& geom_sample, const bool& is_prev_in_np, const double& step_length, const vector<double>& cutoffs_p, const vector<double>& distances, const vector<Point_3D>& affected_points, vector<Point_3D>& new_cnt, Point_3D& new_point)const;
    bool Is_cnt_segment_traversing_gnp(const GNP& gnp_i, const vector<Point_3D>& new_cnt, const Point_3D& new_point, const Point_3D& S, const Point_3D& r_hat)const;
    int Move_point_for_traversing_cnt_segment(const GNP& gnp_i, const vector<Point_3D>& new_cnt, const Point_3D& r_hat, Point_3D& S, Point_3D& new_point)const;
