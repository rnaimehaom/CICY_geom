...
public:
    ...
    int Find_junction_points_for_gnps(const bool& allow_simplex3, const cuboid &sample, const GNP &GNP_A, const GNP &GNP_B, const Point_3D &N, const double &distance, vector<Point_3D> &points_gnp, Point_3D& PointA, Point_3D& PointB);
    int Find_closest_simplices_of_gnps_in_contact(const bool& allow_simplex3, const GNP &GNP_A, const GNP &GNP_B, const Point_3D &N, const double &distance, vector<int> &simplexA, vector<int> &simplexB, int &v_sumA, int &v_sumB);
    int Complete_simplex_of_size_three(set<int>& vertex_set);
